...
function SWEP:CreateGrenade(power)
	if (SERVER) then
		...
		if (IsValid(entity)) then
			...
			local trail = util.SpriteTrail(entity, entity:LookupAttachment("fuse"), Color(0, 255, 255), true, 8, 1, 1, (1 / 9) * 0.5, "sprites/bluelaser1.vmt");
			...
			timer.Simple(3, function()
				if (IsValid(entity)) then
					...
					local flare = ents.Create("cw_flare");
