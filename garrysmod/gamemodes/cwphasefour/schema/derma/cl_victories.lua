...
function PANEL:Rebuild()
	...
	local label = vgui.Create("cwInfoText", self);
		label:SetText("Some victories give you a reward when you achieve them.");
		label:SetInfoColor("blue");
	self.panelList:AddItem(label);
	
	for k, v in pairs(PhaseFour.victory:GetAll()) do
		self.currentVictory = v;
		self.panelList:AddItem(vgui.Create("cw_Victory"));
	end;
...
function PANEL:Init()
	...
	self.nameLabel:SetText(self.victory.name);
	...
	self.spawnIcon:SetToolTip(self.victory.description);
...
function PANEL:Think()
	self.progressLabel:SetText(PhaseFour.victories:Get(self.victory.name).."/"..self.victory.maximum);
	self.progressLabel:SizeToContents();
